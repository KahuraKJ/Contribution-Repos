...
// Check for ID in the URL parameter (from the Modal redirect)
$id_no = $_GET['id_no'] ?? null;

// Handle the case where the script is loaded without a GET parameter.
if (!$id_no && $_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id_no'])) {
    // This allows the old single-entry form to still work, but you requested it to be removed.
    // We will now only rely on the GET parameter.
    $id_no = $_POST['id_no'];
}
...
// --- 1. CORE LOGIC: Check if an ID was passed in the URL ---
if ($id_no) {
    ...
    $sql = "SELECT
            ...
            LEFT JOIN products pr ON p.id_no = pr.id_no
            WHERE p.id_no = ?
            LIMIT 1";

    try {
        if (!isset($con) || !$con) {
            $error_message = "Connection Error: Database connection failed.";
        } else {
            ...
            if (!$member_data) {
                ...
                // NOTE: File upload fallback is still included in case you need it, 
                // but if ID generation is purely modal-driven, this block is optional.
                ...
                if (empty($photo_path)) {
                    $error_message = "No photo found for this member.";
                    $member_data = null; 
                }
            }
        }
    ...
    // If no ID is passed in the URL, display an instruction or error.
    $error_message = "No Member ID provided. Please return to the member summary page and click 'Generate ID Card'.";
}
?>
...
<p class="text-sm text-gray-600 mb-4 print-hidden">Member ID: **<?= htmlspecialchars($id_no); ?>**</p>
...
<div class="flex justify-center items-center space-x-4 mt-8 print-hidden">
    ...
    <button onclick="window.location.href='adminDisplay.php'" 
        class="px-8 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg shadow-lg transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50">
    Home
</button>
</div>
...
<script>
    ...
    async function sendIDCard() {
        ...
        // The member ID is now reliably available from the PHP variable used in the title/display
        const memberID = "<?= htmlspecialchars($id_no ?? ''); ?>"; 
        ...
        // ... (rest of the sendIDCard function remains the same)
